 ...
 * @author Pavi
 */
public class OperationBookingList extends javax.swing.JFrame {
    String id;
    JFrame frame;
    JLabel label;
    JLabel label2;
    ...
    String ID;
    /**
     * Creates new form OperationBookingList
     */
    public OperationBookingList() {
        initComponents();
    }
    
    public OperationBookingList(String ID,JFrame Frame,JLabel Label,JLabel Label2,JPanel Panel) {
        initComponents();
        id=ID;
        frame=Frame;
        label=Label;
        ...
        String sql="select name,surname,phone from patient where id in (select patient_id from patient_doctor where id in (select patient_doctor_id from operation where done=?))";
        try{
        ...
        OperationBookingList close=this;
    ...
    private void jNameKeyTyped(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_jNameKeyTyped
        ...
        ResultSet rs = null;
        con = Link.getConnection();
        String sql="select name,surname,phone from patient where name like '"+start+"%' and id in (select patient_id from patient_doctor where id in (select patient_doctor_id from operation where done=?))";
        try{
            st=con.prepareStatement(sql);
            st.setString(1,"No");
            rs=st.executeQuery();
    ...
    private void jSelectMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jSelectMouseClicked
        ...
        String sql1="select * from patient_stage where patient_id=? and patient_type=?";
        try{
            st1=con1.prepareStatement(sql1);
            st1.setString(1,ID);
            st1.setString(2,"Indoor");
            rs1=st1.executeQuery();
            if(rs1.next()){
                OperationBooking operation = new OperationBooking(id,ID);
                operation.setVisible(true);
                dispose();
            }
            else{
                int response = JOptionPane.showConfirmDialog(this, full_name+" not booked ward. Do you want to book the ward?", "Ward booking", JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE);
                if(response==JOptionPane.YES_OPTION){
                    WardBooking ward = new WardBooking(id);
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(OperationBookingList.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new OperationBookingList().setVisible(true);
            }
        });
    }
